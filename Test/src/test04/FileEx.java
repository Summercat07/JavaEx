...
public class FileEx {
	...
	public static void main(String[] args) {
		...
		File f2 = new File("d:\\java_sample");//새로 만들고자 하는 디렉터리
		if(!f2.exists()) {//f2 디렉터리가 존재하는지 검사
			f2.mkdir();//존재하지 않으면 디렉토리 생성
		}
		
		listDirectory(new File("d:\\"));//d:\\에 있는 파일 리스트 출력
		f2.renameTo(new File("d:\\javasample"));// java_sample ->
													//javasample로 이름변경
		listDirectory(new File("d:\\"));//javasample로 변경한 후 리스트 출력
	}
